...
// NOTES:
// - check: received_motor_state_msg_ptr_
// - check: 
...
CallbackReturn HtbSystem::on_init(const hardware_interface::HardwareInfo& hardware_info)
{
  ...
  {
    RCLCPP_INFO(rclcpp::get_logger("HtbSystem"), "Joint '%s' found", j.name.c_str());
  ...
  {
    RCLCPP_FATAL(rclcpp::get_logger("HtbSystem"), "Joint order size is invalid");
    return CallbackReturn::ERROR;
  }
  ...
  for (auto& j : info_.joints)
  {
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("HtbSystem"), "Joint '%s' missing from velocity command joint order",
                   j.name.c_str());
      return CallbackReturn::ERROR;
    }
  }
...
CallbackReturn HtbSystem::on_activate(const rclcpp_lifecycle::State&)
{
  ...
  motor_state_subscriber_ =
      node_->create_subscription<JointState>("~/motors_response", rclcpp::SensorDataQoS(),
                                             std::bind(&HtbSystem::motor_state_cb, this, std::placeholders::_1));
  ...
  {
    RCLCPP_WARN(rclcpp::get_logger("HtbSystem"), "Feedback message from motors wasn't received yet");
...
void HtbSystem::motor_state_cb(const std::shared_ptr<JointState> msg)
{
...
return_type HtbSystem::read(const rclcpp::Time&, const rclcpp::Duration&)
{
  ...
  if (!motor_state)
  {
    RCLCPP_ERROR(rclcpp::get_logger("HtbSystem"), "Feedback message from motors wasn't received");
  ...
  for (auto i = 0u; i < motor_state->name.size(); i++)
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("HtbSystem"), "Position or velocity feedback not found for joint %s",
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("HtbSystem"), "Position feedback: %f, velocity feedback: %f",
...
return_type HtbSystem::write(const rclcpp::Time&, const rclcpp::Duration&)
{
  ...
  if (realtime_motor_command_publisher_->trylock())
  {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("HtbSystem"), "Wrtiting motors cmd message");
